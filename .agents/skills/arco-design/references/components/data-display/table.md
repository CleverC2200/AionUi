  ...
  summary={(currentData) => (
    <Table.Summary>
      <Table.Summary.Row>
        ...
        <Table.Summary.Cell>{currentData.reduce((sum, row) => sum + row.amount, 0)}</Table.Summary.Cell>
      </Table.Summary.Row>
    </Table.Summary>
  )}
/>
...
const columns = [
  {
    title: '名称',
    dataIndex: 'name',
    render: (col, record) => <EditableCell value={col} onChange={(v) => updateRecord(record.key, 'name', v)} />,
  },
];
```
