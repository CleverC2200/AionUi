...
<Tag checkable checked={checked} onCheck={setChecked}>
  筛选项
</Tag>;

// 多种预设颜色
{
  ['red', 'orangered', 'orange', 'gold', 'lime', 'green', 'cyan', 'blue', 'arcoblue', 'purple', 'magenta'].map(
    (color) => (
...
<Space>
  ...
  {inputVisible ? (
    ...
    <Tag icon={<IconPlus />} onClick={() => setInputVisible(true)}>
      添加
    </Tag>
  )}
</Space>;
