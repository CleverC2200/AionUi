...
- 按变更爆炸半径选择门禁：开发中先运行最接近的定向测试或静态检查；只有变更触及跨包共享契约、依赖或锁文件、构建发布链、平台边界或 release candidate 时，才升级为相关全量或跨平台门禁。文档和低风险局部改动不运行无关大型套件。
- 同一 SHA 已通过的完整门禁直接复用记录；只有代码、依赖、配置或目标基线变化后才重跑。失败时修复实现、测试或门禁配置，不得通过删减检查范围或弱化断言绕过。
- 运行昂贵构建或全量门禁前检查剩余磁盘，优先复用共享缓存和已有产物；只清理本任务拥有且可再生的输出，不清理其他工作树或正在使用的缓存。
- 用户要求本地生成并交付桌面安装包时，默认只生成 macOS DMG 与 Windows NSIS EXE；除非用户当轮明确要求，不生成或保留 macOS ZIP 及其 blockmap。官方发布或自动更新流程明确依赖 ZIP 时不适用此约定。
