...
- **频道：**新增 Slack 频道配置界面（官方 #3935）
- **文件浏览器：**新增复制相对路径和绝对路径的右键菜单（官方 #3929）
- **源代码管理：**变更面板支持折叠分区及树形/列表视图（官方 #3926）

#### 问题修复

- **助手：**由编辑器驱动 Antigravity（官方 #3951）
- **构建：**合并 React 依赖分包，修复白屏问题（官方 #3938）
- **聊天：**复制按钮现在复制完整的 AI 回复，而不只是最后一个文本片段（官方 #3949）
- **会话：**项目会话在窄屏下也能正常显示预览（官方 #3934）
- **文件浏览器：**文件系统订阅失败后自动重新订阅（官方 #3954）
- **打包：**不再要求内置 Claude/Codex，并统一版本差异处理（官方 #3916）
- **预览：**通过 `fileRef` 正确解析项目 Markdown 中的相对图片路径（官方 #3948）
- **输入框：**添加文件夹或项目根目录时生成文件标签（官方 #3869）
- **技能：**WebUI 支持浏览技能文件（官方 #3946）

#### 重构

- **主题：**移除旧版主题迁移逻辑（官方 #3918）

### GEAUi 定制改动

#### 新功能

- **工作区与认证：**完善 GEAUi 品牌、会话工作区、Agent Hub 与飞书扫码登录体验（个人仓库 #1）
- **MCP：**新增带身份认证的 GEA 网关（个人仓库 #2）
- **模型：**从 GEA 自动同步个人模型配置（个人仓库 #4）
- **聊天：**长任务执行计划支持悬浮常驻和状态展示（个人仓库 #6）
- **项目管理：**侧边栏支持创建、管理和切换项目（个人仓库 #12）
- **会话资源：**新增来源与输出面板，统一预览会话文件和产物（个人仓库 #15）
- **实时语音：**新增托管实时语音交互体验（个人仓库 #17）
- **团队协作：**新增工作控制台及稳定的 E2E 覆盖（个人仓库 #28）

#### 问题修复

- **文件预览：**工作区文件改为在标签页中打开（个人仓库 #3）
- **模型：**登录后自动刷新 GEA 配置，并明确提示缺失的本地密钥（个人仓库 #8、#13）
- **品牌：**统一 Aion CLI 与 GEAUi 图标（个人仓库 #9）
- **认证：**使用系统安全存储恢复飞书登录状态（个人仓库 #11）
- **安装程序：**统一 Windows 可执行文件名称校验（个人仓库 #14）
- **侧边栏：**项目列表与会话工作区保持一致（个人仓库 #16）
- **模型调用：**首次 GEA 请求强制发起工具调用，并兼容后续流式事件（个人仓库 #18）

#### 工程与交付

- **构建：**校验 GEAUi 安装产物并支持手动触发 macOS/Windows 构建（个人仓库 #5、#7）
- **E2E：**同步官方主分支并更新测试兼容契约（个人仓库 #29）

### 核心服务（[v0.1.63](https://github.com/iOfficeAI/AionCore/releases/tag/v0.1.63)）

#### 新功能

- **频道：**新增 Slack Socket Mode 插件（官方 #806）
- **文件系统：**新增服务端复制绝对路径到剪贴板的接口（官方 #803）
- **源代码管理：**支持在工作区根目录下进行一级仓库发现（官方 #800）

#### 问题修复

- **Agent：**避免空闲扫描器终止仍有后台任务运行的 Agent（官方 #811）
- **项目：**向 Agent 传递保留真实大小写的绝对路径（官方 #809）

#### 重构

- **会话：**运行用户自己的 Claude/Codex，并统一版本差异处理流程（官方 #799）
