# GEA Headless Server Deployment Guide

Deploy GEA WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- GEA `.deb` package from [Releases](https://github.com/iOfficeAI/AionUi/releases)
...
GEA is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
> **Tip**: `WORKDIR` determines the directory GEA can access for file operations. Set it to your project workspace.

---

## Remote Access

GEA WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for GEA (Electron / Chromium Layer)
...
### Step 4: GEA Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside GEA WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by GEA's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| GEA can only access `/tmp`                | Set `WORKDIR` in the startup script to your workspace path   |
...
在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 GEA WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- GEA `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/AionUi/releases)）
...
GEA 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
GEA WebUI 监听端口 **25808**，根据网络环境选择访问方式：
