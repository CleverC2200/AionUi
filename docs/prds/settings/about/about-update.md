...
- If a silent `--updated` install cannot close GEA after retries, the installer writes `%APPDATA%\AionUi\installer-last-failure.json`.
...
- Run silent `/S --updated` while GEA cannot be closed. Expected: marker file appears under `%APPDATA%\AionUi`; next app launch shows the update failure notification; the following launch does not show it again.
