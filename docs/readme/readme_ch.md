...
[✨ Cowork 演示](#-cowork-演示) ·
[🤔 为什么选择 GEAUi？](#-为什么选择-aionui-而非-claude-cowork) ·
[🚀 快速开始](#-快速开始) ·
[💬 社区](#-社区与支持)
...
- **MCP 统一管理** — 在一处管理 MCP（模型上下文协议）工具；GEAUi 会根据各 Agent 的能力注入或同步兼容的传输方式
...
- **多 Agent 并行执行** — Leader 将任务拆分并委派给并行运行的 Teammate；外部 Agent 通过 ACP（Agent Client Protocol）连接，由 GEAUi 负责团队协调
...
- **🧭 GEAUi Butler** — 在 GEAUi 内提供产品使用指导、配置帮助和故障排查

**自定义技能**：在 **设置 → Skills** 中导入或创建技能，再按需绑定到助手。技能来源分三层：内置（随 GEAUi 附带）、自定义（你自己的）以及扩展（通过扩展 SDK 贡献）。内置技能有 `pptx`、`docx`、`pdf`、`xlsx`、`mermaid` 等。

> 💡 内置定义的权威来源是 GEACore 的[助手目录](https://github.com/iOfficeAI/AionCore/blob/main/crates/aionui-app/assets/builtin-assistants/assistants.json)；助手规则仍使用 Markdown，并可在 GEAUi 中自定义。
