 ...
 *  2. Legacy AIONUI_AIONCORE_BINARY override (development compatibility)
...
const LEGACY_BIN_ENV_VAR = 'AIONUI_AIONCORE_BINARY';
...
export function resolveBinaryPath(): string {
  ...
  const legacyOverride = process.env[LEGACY_BIN_ENV_VAR]?.trim();
  if (legacyOverride && existsSync(legacyOverride)) return legacyOverride;
