 ...
 *
 * MCP 已随应用解包，直接由当前 Node 进程执行；这样首次启动不依赖 npm 网络或缓存，
 * Windows 也不再需要经过 npx.cmd/cmd.exe。
 *
 ...
 *
 * The MCP ships unpacked with the app and runs under the current Node executable. First launch
 * therefore has no npm network/cache dependency and Windows needs no npx.cmd/cmd.exe bridge.
 */
export const buildMcpSpawnCommand = (deps: {
  ...
  return {
    ...
    args: [deps.entryPath, '--browser-url', deps.browserUrl],
  };
};
