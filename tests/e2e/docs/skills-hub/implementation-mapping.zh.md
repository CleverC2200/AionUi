# Skills Hub E2E 历史实现映射

> 注意：本文是 2026-04-21 的历史快照，不再代表当前 Skills 设置页。当前页面已用“添加技能”的文件夹、父目录和 ZIP 导入替代外部源 Tab、持久路径、刷新与导出流程；因此 TC-S-08~10、TC-S-12~21、TC-S-26 已从可执行套件移除。当前文件夹导入主流程由 `manual-import.e2e.ts` 覆盖，实际测试清单以 Playwright `--list` 输出为准。
