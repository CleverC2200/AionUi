...
- v1.4：外部源 Tab、持久路径、手动刷新和导出流程退出当前产品；TC-S-08~10、TC-S-12~21、TC-S-26 仅保留为历史设计，不再属于可执行回归。当前文件夹导入由 TC-S-29 覆盖；TC-S-27/28 仅在运行环境注册了对应技能时执行。
...
- ✅ 扩展技能卡片显示 Extension 标签和 Puzzle 图标（紫色）
...
- ✅ 自动注入技能卡片显示 Auto 标签和 Lightning 图标（绿色）

**断言类型**：UI 断言 + Bridge 断言

- UI：验证板块容器存在、标题正确、布局结构完整
- Bridge：查询自动技能列表，确认运行环境满足执行前置条件
...
**文档状态**：✅ v1.4（历史设计与当前可执行回归边界已标注）
**修订说明**：退役已移除功能对应的用例；TC-S-27/28 根据运行环境中的扩展技能和自动注入技能注册情况执行。
**下一步**：以 Playwright `--list` 和当前 E2E 运行结果作为可执行清单。
