...
const { getMock, itemsMock, unarchiveMock } = vi.hoisted(() => ({
  getMock: vi.fn(),
  itemsMock: vi.fn(),
  unarchiveMock: vi.fn(),
}));
...
vi.mock('@/common', () => ({
  ipcBridge: {
    sidebar: {
      ...
      unarchive: { invoke: unarchiveMock },
...
describe('ArchivedSettings paging', () => {
  beforeEach(() => {
    getMock.mockReset();
    itemsMock.mockReset();
    unarchiveMock.mockReset().mockResolvedValue(undefined);
  });
  ...
  it('drops appended windows when an archive mutation refetches the first screen', async () => {
    getMock
      ...
      .mockResolvedValueOnce({
        groups: [projectGroup('P1', 'Alpha', [conv('c3', 'Chat Three')], false)],
        has_more_groups: false,
      });
    itemsMock.mockResolvedValue({ items: [conv('c2', 'Chat Two')], has_more: false, next_cursor: undefined });
    ...
    fireEvent.click(await screen.findByText('settings.archived.loadMore'));
    await screen.findByText('Chat Two');
    fireEvent.click(screen.getAllByText('settings.archived.restore')[0]);

    await screen.findByText('Chat Three');
